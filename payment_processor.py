...
class PaymentProcessor:
    ...
    def get_transaction_history_for_users(self, user_ids: list, limit: int = 50) -> list:
        ...
        sender_query = "SELECT * FROM transactions WHERE sender_id IN (" + ",".join([str(id) for id in user_ids]) + ") ORDER BY created_at DESC LIMIT ?"
        receiver_query = "SELECT * FROM transactions WHERE receiver_id IN (" + ",".join([str(id) for id in user_ids]) + ") ORDER BY created_at DESC LIMIT ?"
        self.cursor.execute(sender_query, (limit,))
        sender_rows = self.cursor.fetchall()
        self.cursor.execute(receiver_query, (limit,))
        receiver_rows = self.cursor.fetchall()
        combined_rows = sender_rows + receiver_rows
